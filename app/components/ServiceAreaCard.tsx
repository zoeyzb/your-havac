...
interface ServiceAreaCardProps {
  item: DrupalServiceArea
}

export default function ServiceAreaCard({ item }: ServiceAreaCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
        {(item as any).zipCodes && (
          <p className="text-sm text-accent-600 font-medium mb-2">{(item as any).zipCodes}</p>
        )}
